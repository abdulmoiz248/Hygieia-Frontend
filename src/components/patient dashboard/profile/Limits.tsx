...
export default function Limits({ profile, setProfile, isEditing }: LimitsProps) {
  // Define keys as an array of keyof ProfileType['limit']
  const limitFields: { key: keyof ProfileType["limit"]; label: string }[] = [
          ...
          <div key={key} className="flex flex-col">
            ...
            <Input
              ...
              onChange={(e) =>
                setProfile((prev) => ({
                  ...prev,
                  limit: { ...prev.limit, [key]: Number(e.target.value) },
                }))
              }
              disabled={!isEditing}
              min="0"
