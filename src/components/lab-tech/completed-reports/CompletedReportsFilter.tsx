...
import { Search, Filter, X, ChevronDown, MapPin, Calendar, FileText, SortAsc } from "lucide-react"
...
export function CompletedReportsFilter({ 
  searchQuery, 
  setSearchQuery,
  locationFilter,
  setLocationFilter,
  ...
  const activeFilterCount = [locationFilter, dateFilter, typeFilter].filter(f => f !== "all").length + (sortBy !== "newest" ? 1 : 0)
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-4 border-t border-cool-gray/20">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {["all", "today", "yesterday", "this-week", "this-month"].map((date) => (
                    <button
                      key={date}
                    ...
                    >
                      {date === "all" ? "All" : date === "this-week" ? "This Week" : date === "this-month" ? "This Month" : date.charAt(0).toUpperCase() + date.slice(1)}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-dark-slate-gray uppercase tracking-wide flex items-center gap-2">
                  <FileText className="w-3.5 h-3.5 text-soft-blue" />
                  Test Type
                </label>
                <div className="flex flex-wrap gap-2">
                  {["all",  "scan", "report", ].map((type) => (
                    <button
                      key={type}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-dark-slate-gray uppercase tracking-wide flex items-center gap-2">
                  <SortAsc className="w-3.5 h-3.5 text-soft-coral" />
                  Sort By
                </label>
                <div className="flex flex-wrap gap-2">
                  {[
