...
export function DoctorProfile({ id }: { id: string }) {
  ...
  const doctor: Doctor | undefined = doctors?.find((item) => item.id === id)

  if (!doctor) {
    notFound()
  }

  const profileUrl = `${typeof window !== "undefined" ? window.location.origin : ""}/doctors/${doctor.id}`
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out ${doctor.name}, a ${doctor.specialization} specialist`,
          url: profileUrl,
        })
      } catch (error) {
                ...
                <div className="flex flex-col sm:flex-row items-start gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                      <div>
                        <h1 className="text-3xl font-bold text-soft-coral mb-2">{doctor.name}</h1>
                        ...
                        <Dialog open={showQR} onOpenChange={setShowQR}>
                          ...
                          <DialogContent className="sm:max-w-md">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Scan this QR code to share {doctor.name}&apos;s profile
                              </DialogDescription>
                            </DialogHeader>
                    ...
                    <p className="text-cool-gray text-lg leading-relaxed mb-6">{doctor.bio}</p>
              ...
              <Card className="bg-gradient-to-br from-soft-blue/10 via-mint-green/5 to-cool-gray/10 backdrop-blur-lg border border-border/30 shadow-lg rounded-2xl">
                ...
                <CardContent>
                  <ul className="space-y-3 list-disc list-inside text-dark-slate-gray">
                    {doctor.education.map((educationItem, index) => (
