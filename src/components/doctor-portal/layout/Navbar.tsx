...
import useDoctorNotifications from "@/hooks/doctor/useDoctorNotifications"
import { UseQueryResult } from "@tanstack/react-query"
import { DoctorNotification } from "@/hooks/doctor/useDoctorNotifications"
...
export function DoctorTopNav({ onMobileMenuToggle }: TopNavProps) {
  const user = useDoctorStore().profile

  const {
    data: notifications,
  }: UseQueryResult<DoctorNotification[]> = useDoctorNotifications(
    user?.id || ""
  )
  ...
  const userInitials = (user?.name ?? "Doctor")
    .split(" ")
    .map((n) => n[0])
    .join("")
    .toUpperCase()
